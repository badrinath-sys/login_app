...
class JSqueeze {
    ...
    protected function extractStrings($f) {
                ...
                switch ($f[$i]) {
                    ...
                    case '/':
                        if ('*' == $f[$i + 1]) {
                            ...
                            if (false !== strpos('-!%&;<=>~:^+|,()*?[{} ', $a) || (false !== strpos('oenfd', $a) && preg_match(
                    ...
                    case "\n":
                        if ($j > 3) {
                            ...
                            $code[++$j] = false !== strpos('kend+-', $code[$j - 1]) && preg_match(
        ...
        foreach ($strings as $r1 => &$r2) {
            $r2 = "/'" == substr($r1, -2) ? str_replace(array("\\'", '\\"'), array("'", '"'), $r2) : str_replace('\\' . $q[1], $q[1], $r2);
        }
    ...
    protected function makeVars($closure, &$tree, $key) {
        ...
        if (preg_match_all("#//''\"\"[0-9]+(?:['!]|/')#", $closure, $w)) {
            foreach ($w[0] as $a) {
                $v = "'" === substr($a, -1) && "/'" !== substr($a, -2) && $this->specialVarRx ? preg_split("#([.,{]?(?:[gs]et )?(?<![a-zA-Z0-9_\$@]){$this->specialVarRx}:?)#", $this->strings[$a], -1, PREG_SPLIT_DELIM_CAPTURE) : array($this->strings[$a]);
                $a = count($v);
    ...
    protected function getNewName($m) {
        ...
        $post = (isset($this->reserved[$m]) ? ('true' === $m ? '!0' : ('false' === $m ? '!1' : $m)) : (
                isset($this->local_tree[$m]) ? $this->local_tree[$m] : (
                isset($this->used_tree[$m]) ? $this->used_tree[$m] : $m
                )
                )
                ) . $post;
